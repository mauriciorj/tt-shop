                ...
                if response.status_code == 200:
                    with open(image_path, 'wb') as f:
                        for chunk in response.iter_content(1024):
                            f.write(chunk)
                    print(f'[ SELENIUM ] downloaded image for product: {store_id}')
                else:
                    print(f"[ SELENIUM ] Failed to download image for product {store_id} (k_id: {k_id}). Status: {response.status_code}")
            except Exception as e_img:
                print(f"[ SELENIUM ] Error downloading image for product {store_id}: {e_img}")
