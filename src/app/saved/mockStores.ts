const generateRevenueHistory = (baseRevenue: number): number[] => {
  return Array.from({ length: 7 }, () =>
    Math.round(baseRevenue * (0.8 + Math.random() * 0.4))
  )
}

export const mockStores: any[] = [
  {
    id: 1,
    rank: 1,
    name: 'GlowUp Beauty',
    category: 'Beauty',
    followers: '12.5M',
    products: 245,
    revenue: '$2.8M',
    revenueHistory: generateRevenueHistory(400000),
    trend: 'up',
    trendValue: 15,
    avatar:
      'https://images.unsplash.com/photo-1522335789203-aabd1fc54bc9?w=100&h=100&fit=crop',
  },
  {
    id: 2,
    rank: 2,
    name: 'TechTrend Hub',
    category: 'Electronics',
    followers: '8.2M',
    products: 189,
    revenue: '$2.1M',
    revenueHistory: generateRevenueHistory(300000),
    trend: 'up',
    trendValue: 12,
    avatar:
      'https://images.unsplash.com/photo-1519389950473-47ba0277781c?w=100&h=100&fit=crop',
  },
  {
    id: 3,
    rank: 3,
    name: 'FashionForward',
    category: 'Fashion',
    followers: '7.8M',
    products: 412,
    revenue: '$1.9M',
    revenueHistory: generateRevenueHistory(270000),
    trend: 'stable',
    trendValue: 2,
    avatar:
      'https://images.unsplash.com/photo-1441986300917-64674bd600d8?w=100&h=100&fit=crop',
  },
  {
    id: 4,
    rank: 4,
    name: 'HomeStyle Living',
    category: 'Home & Garden',
    followers: '5.6M',
    products: 328,
    revenue: '$1.5M',
    revenueHistory: generateRevenueHistory(215000),
    trend: 'up',
    trendValue: 8,
    avatar:
      'https://images.unsplash.com/photo-1556909114-f6e7ad7d3136?w=100&h=100&fit=crop',
  },
  {
    id: 5,
    rank: 5,
    name: 'FitLife Pro',
    category: 'Sports',
    followers: '4.9M',
    products: 156,
    revenue: '$1.2M',
    revenueHistory: generateRevenueHistory(170000),
    trend: 'up',
    trendValue: 22,
    avatar:
      'https://images.unsplash.com/photo-1571019613454-1cb2f99b2d8b?w=100&h=100&fit=crop',
  },
  {
    id: 6,
    rank: 6,
    name: 'PetPals Paradise',
    category: 'Pets',
    followers: '4.2M',
    products: 287,
    revenue: '$980K',
    revenueHistory: generateRevenueHistory(140000),
    trend: 'up',
    trendValue: 18,
    avatar:
      'https://images.unsplash.com/photo-1587300003388-59208cc962cb?w=100&h=100&fit=crop',
  },
  {
    id: 7,
    rank: 7,
    name: 'GamersGalaxy',
    category: 'Gaming',
    followers: '3.8M',
    products: 134,
    revenue: '$890K',
    revenueHistory: generateRevenueHistory(127000),
    trend: 'down',
    trendValue: -3,
    avatar:
      'https://images.unsplash.com/photo-1538481199705-c710c4e965fc?w=100&h=100&fit=crop',
  },
  {
    id: 8,
    rank: 8,
    name: 'Organic Kitchen',
    category: 'Food',
    followers: '3.5M',
    products: 198,
    revenue: '$820K',
    revenueHistory: generateRevenueHistory(117000),
    trend: 'up',
    trendValue: 5,
    avatar:
      'https://images.unsplash.com/photo-1490645935967-10de6ba17061?w=100&h=100&fit=crop',
  },
  {
    id: 9,
    rank: 9,
    name: 'Kids Kingdom',
    category: 'Kids & Baby',
    followers: '3.1M',
    products: 445,
    revenue: '$750K',
    revenueHistory: generateRevenueHistory(107000),
    trend: 'stable',
    trendValue: 1,
    avatar:
      'https://images.unsplash.com/photo-1515488042361-ee00e0ddd4e4?w=100&h=100&fit=crop',
  },
  {
    id: 10,
    rank: 10,
    name: 'Jewelry Box',
    category: 'Jewelry',
    followers: '2.9M',
    products: 312,
    revenue: '$680K',
    revenueHistory: generateRevenueHistory(97000),
    trend: 'down',
    trendValue: -2,
    avatar:
      'https://images.unsplash.com/photo-1515562141207-7a88fb7ce338?w=100&h=100&fit=crop',
  },
]
